...
function ContainerDot({ container }: { container: ContainerInfo }) {
  ...
  return (
    ...
    >
      <div className="absolute bottom-[calc(100%+6px)] left-1/2 -translate-x-1/2 bg-bg-card border border-border rounded px-2 py-1 text-[10px] font-mono text-text-primary whitespace-nowrap z-50 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
        {container.name}
      </div>
    </div>
  );
...
function ProjectCluster({ project }: { project: ProjectGroup }) {
  return (
    <div className="bg-bg-card border border-border-subtle rounded-md px-2.5 py-2 flex items-center gap-1.5 transition-all hover:border-border hover:bg-bg-card-hover">
      <span className="text-[11px] text-text-secondary font-medium mr-1">{project.name}</span>
...
export default function ContainerGrid({ servers }: ContainerGridProps) {
  return (
    <div className="bg-bg-surface border border-border-subtle rounded-lg p-4">
      <div className="text-[11px] font-medium uppercase tracking-[1px] text-text-secondary mb-3.5">
        Container Status
      </div>
      {servers.map((server) => (
        <div key={server.server} className="mb-4 last:mb-0">
          <div className="text-[13px] font-semibold text-text-primary mb-2.5 flex items-center gap-2">
            {server.server}
            <span className="font-mono text-[11px] text-text-tertiary font-light">
